	...
	<body>
		...
		<script type="text/javascript">
			...
			function chartTAKOEE() {
				...
				for ( var i = 1; i < myTable.rows.length; i++ ) {
				    ...
				    produktionstid.push(myTable.rows[i].cells[2].innerHTML);
				    stopptid.push(myTable.rows[i].cells[3].innerHTML);
			...
			function chartProduktion() {
				...
				for ( var i = 0; i < 24; i++ ) {
					for ( var x = 1; x < myTable.rows.length; x++ ) {
						var d = new Date(myTable.rows[x].cells[26].innerHTML);
						var weekday = d.getDay();
						// document.write(weekday);
						// if (weekday === 0 || weekday === 6) {
						//
						// }
						temp = temp + parseInt(myTable.rows[x].cells[i].innerHTML);
					}

					kl.push(temp / (myTable.rows.length - 1));
					temp = 0;
		...
		<?php
			...
			function TAKOEE() {
				...
				echo "</tr></table>";
				echo '<script type="text/javascript">chartTAKOEE();</script>';
			}
