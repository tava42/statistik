		...
		<?php
			...
			function TAKOEE($dashboard) {
				...
				if ($dashboard == false) {
					...
					// 'Max prod', 'Avg prod',
					'Produktionstid', 'Stopptid', 'Kass', 'Tillgänglighet', 'Anläggningsutbyte', 'Kvalite', 'OEE');
					echo "<div class='prodChart'><canvas id='prodChart'></canvas></div>";
					echo "<table style='width:100%;' id='myTable'>";
					create_table($headers);
					echo "<br>";
					...
					foreach ($data as $row ) {
						...
						$produktionstid = ((strtotime($row["date_end"]) - strtotime($row["date_start"])) / 60) - $row["idle"];
						$kass = $row['kass'];
						$avg_prod = $produktion / ($produktionstid / 60);
						...
						if ($produktionstid != 0 && $produktion != 0) {
							...
							<td>" . bcdiv(($produktionstid / 60),1, 2)  . "</td><td>" . bcdiv(($stopptid / 60),1, 2) . "</td><td>" .
